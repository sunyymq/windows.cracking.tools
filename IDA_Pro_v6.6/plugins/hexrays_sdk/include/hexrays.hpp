 ...
 *      Copyright (c) 2007-2009 by Hex-Rays, support@hex-rays.com
 *      ALL RIGHTS RESERVED.
 */
...
//@}
  ...
  /// Get number radix
  /// \return 2,8,10, or 16
  int getRadix(void) const { return ::getRadix(flags, opnum); }
  ...
  bool isHex(void) const { return ::isNum(flags, opnum) && getRadix() == 16; }
  /// Is a decimal number?
  bool isDec(void) const { return ::isNum(flags, opnum) && getRadix() == 10; }
  /// Is a octal number?
  bool isOct(void) const { return ::isNum(flags, opnum) && getRadix() == 8; }
  /// Is a symbolic constant?
  bool isEnum(void) const { return ::isEnum(flags, opnum); }
  /// Is a character constant?
  bool isChar(void) const { return ::isChar(flags, opnum); }
  /// Is a structure field offset? (not used yet)
  bool isStroff(void) const { return ::isStroff(flags, opnum); }
  /// Is a number?
  bool isNum(void) const { return !isEnum() && !isChar() && !isStroff(); }
...
///  \param bufsize size of output buffer. must be bigger than 4 bytes (at least)
...
struct lvar_locator_t
{
  ...
  /// Calculate the variable location (only for reg1 and stkoff variables)
...
public:
  ...
  {
    tif = t;
    setflag(flags, CVAR_FLOAT, t.is_floating()); // set_floating_var/clr_floating_var
  }
  /// Set variable type
  /// If the type is inacceptable, interr is generated.
  void hexapi set_lvar_type(const tinfo_t &t);
  ...
  /// the variable type.
  void hexapi set_width(int w, bool is_float=false);
...
/// Saved user settings for local variables
...
struct user_lvar_visitor_t
{
  ...
  int flags;
/// \defgroup ULV_ user_lvar_visitor_t property bits
/// Used in user_lvar_visitor_t::flags
//@{
#define ULV_PRECISE_DEFEA 0x0001        ///< Use precise defea's for lvar locations
//@}

  user_lvar_visitor_t(void) : stkoff_delta(0), flags(ULV_PRECISE_DEFEA) {}

  /// Visit user defined local variable information.
  /// This function is called while we retrieve information from the idb.
  /// \param lv saved information about variable
  virtual int idaapi handle_retrieved_info(const lvar_saved_info_t &lv) = 0;

  /// Visit user defined local variable information.
  /// This function is called while we retrieve information from the idb.
  /// \param lm saved information about variable mappings. may be modified.
  virtual int idaapi handle_retrieved_mapping(lvar_mapping_t &lm) = 0;

  /// Get number of lvars with user defined settings.
  /// This function is called while we save information to the idb.
  /// It resets the enumeration.
  /// \return number of user-defined lvars
  virtual int idaapi get_info_qty_for_saving(void) = 0;

  /// Get next user-defined lvar data
  /// This function is called while we save information to the idb.
  /// \param lv information to save about variable
  /// \returns true is success
  virtual bool idaapi get_info_for_saving(lvar_saved_info_t *lv) = 0;

  /// Get lvar mapping info
  /// This function is called while we save information to the idb.
  /// \returns lvar mapping or NULL
  virtual const lvar_mapping_t *idaapi get_info_mapping_for_saving(void) = 0;
};

/// Restore user defined local variable settings in the database.
/// \param func_ea entry address of the function
/// \param ulv helper visitor class for lvar settings
/// \return 0 if all lvar settings have been enumerated. Otherwise returns
///           the code returned by the visitor class.

int hexapi restore_user_lvar_settings(ea_t func_ea, user_lvar_visitor_t &ulv);


/// Save user defined local variable settings into the database.
/// \param func_ea entry address of the function
/// \param uvl user lvar setting enumerator. get_user_lvar_qty() and get_next_user_lvar()
///            will be used to enumerate user settings.

void hexapi save_user_lvar_settings(ea_t func_ea, user_lvar_visitor_t &ulv);
...
enum warnid_t
{
  ...
  WARN_SPLIT_MACRO,   // 29 replaced macro-insn by simple insns, please redecompile
...
/// \return pointer to version string. For example: "1.0.071025"
...
bool hexapi close_pseudocode(TForm *f);
...
#define MERR_UNKTYPE   (-14)   ///< undefined type %s (currently unused)
...
#define MERR_MAX_ERR   25
#define MERR_LOOP      (-26)   ///< internal code: redo last loop (never reported)
//@}
...
struct hexrays_failure_t
{
  ...
  hexrays_failure_t(void) : code(MERR_OK) {}
...
enum ctype_t
{
  ...
  cot_last     = cot_helper,
  cit_empty    = 69,  ///< instruction types start here
  cit_block    = 70,  ///< block-statement: { ... }
  cit_expr     = 71,  ///< expression-statement: expr;
  cit_if       = 72,  ///< if-statement
  cit_for      = 73,  ///< for-statement
  cit_while    = 74,  ///< while-statement
  cit_do       = 75,  ///< do-statement
  cit_switch   = 76,  ///< switch-statement
  cit_break    = 77,  ///< break-statement
  cit_continue = 78,  ///< continue-statement
  cit_return   = 79,  ///< return-statement
  cit_goto     = 80,  ///< goto-statement
  cit_asm      = 81,  ///< asm-statement
  cit_end
};
...
struct cnumber_t
{
  ...
  size_t hexapi print(char *buf, size_t bufsize, const tinfo_t &type) const;
...
enum item_preciser_t
{
  ...
  // update get_preciser_text() if this enum changes
...
#define EXFL_ALL     0x000F ///< all currently defined bits
//@}
  ...
  /// \return true if the expression has a child with the specified opcode.
  bool hexapi contains_operator(ctype_t needed_op) const;
  ...
  bool contains_comma(void) const { return contains_operator(cot_comma); }
  /// Does the expression contain an embedded statement operator?
  bool contains_insn(void) const { return contains_operator(cot_insn); }
  ...
  bool contains_comma_or_insn_or_label(void) const { return contains_comma() || contains_insn_or_label(); }
  /// Is nice expression?
  ...
  /// \param np Pointer to the variable where the expression value is returned.
  ...
  bool maybe_ptr(void) const
  {
    ...
    if ( get_const_value(&val)
      && (ea_t(val) != val || !isEnabled((ea_t)val)) )
      return false;
    return true;
  }
  ...
  {
    if ( x->type.is_ptr_or_array() ) return x;
    if ( y->type.is_ptr_or_array() ) return y;
    return NULL;
  }
...
struct casm_t : public eavec_t
{
  ...
  void genasm(ea_t ea, char *buf, size_t bufsize) const;
};
...
struct cinsn_t : public citem_t
{
  ...
  /// \return true if the specified opcode has been found
  bool hexapi contains_insn(ctype_t type) const;
...
cexpr_t *hexapi dereference(cexpr_t *e, int ptrsize, bool is_float=false);
...
typedef std::map<cinsn_t *, areaset_t> boundaries_t;
...
const int HEXRAYS_API_MAGIC2 = 0xBE3BBABE; // xxNNxxxx
...
enum hexrays_event_t
{
  ...
  hxe_right_click,      ///< Mouse right click. We can add menu items now.
                        ///< vdui_t *vu
...
struct vdui_t
{
  TForm *form;          ///< attention, may point to a destroyed form
                        ///< check the visibility before proceeding.
  int flags;            ///< \ref VDUI_
...
//@}
  ...
  TCustomControl *ct;   ///< pseudocode view
  TCustomControl *cv;   ///< codeview control (used to display line numbers and breakpoints)
  ...
  cnumber_t *hexapi get_number(void) const;
...
{
  return callui(ui_clearbreak, HEXRAYS_API_MAGIC1, HEXRAYS_API_MAGIC2, &hexdsp, flags).i == HEXRAYS_API_MAGIC1;
}
...
//-------------------------------------------------------------------------
/// Get reference to the current map value
inline areaset_t &boundaries_second(boundaries_iterator_t p)
{
  return *(areaset_t *)hexdsp(hx_boundaries_second, &p);
}
...
//-------------------------------------------------------------------------
/// Insert new (cinsn_t *, areaset_t) pair into boundaries_t
inline boundaries_iterator_t boundaries_insert(boundaries_t *map, const cinsn_t * &key, const areaset_t &val)
{
  boundaries_iterator_t p;
...
{
  return (int)hexdsp(hx_operand_locator_t_compare, this, &r);
}
...
inline AS_PRINTF(3, 4) int vd_printer_t::print(int indent, const char *format, ...)
{
  va_list va;
  va_start(va, format);
  int retval = (int)hexdsp(hx_vd_printer_t_print, this, indent, format, va);
  va_end(va);
  return retval;
}
...
inline AS_PRINTF(3, 4) int qstring_printer_t::print(int indent, const char *format, ...)
{
  va_list va;
  va_start(va, format);
  int retval = (int)hexdsp(hx_qstring_printer_t_print, this, indent, format, va);
  va_end(va);
  return retval;
}
...
{
  return (int)hexdsp(hx_lvar_locator_t_compare, this, &r);
}
...
//--------------------------------------------------------------------------
inline void lvar_t::set_lvar_type(const tinfo_t &t)
{
  hexdsp(hx_lvar_t_set_lvar_type, this, &t);
}

//--------------------------------------------------------------------------
inline void lvar_t::set_width(int w, bool is_float)
{
  hexdsp(hx_lvar_t_set_width, this, w, is_float);
}

//--------------------------------------------------------------------------
inline int lvars_t::find_stkvar(int32 spoff, int width)
{
  return (int)hexdsp(hx_lvars_t_find_stkvar, this, spoff, width);
}
...
{
  return (int)hexdsp(hx_lvars_t_find_lvar, this, &location, width, defblk);
}

//--------------------------------------------------------------------------
inline int restore_user_lvar_settings(ea_t func_ea, user_lvar_visitor_t &ulv)
{
  return (int)hexdsp(hx_restore_user_lvar_settings, &func_ea, &ulv);
}

//--------------------------------------------------------------------------
inline void save_user_lvar_settings(ea_t func_ea, user_lvar_visitor_t &ulv)
{
  hexdsp(hx_save_user_lvar_settings, &func_ea, &ulv);
}
...
//--------------------------------------------------------------------------
inline bool close_pseudocode(TForm *f)
{
...
{
  return (type_sign_t)hexdsp(hx_get_op_signness, op);
}
...
//--------------------------------------------------------------------------
inline size_t cnumber_t::print(char *buf, size_t bufsize, const tinfo_t &type) const
{
  return (size_t)hexdsp(hx_cnumber_t_print, this, buf, bufsize, &type);
}
...
{
  return (int)hexdsp(hx_cnumber_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int var_ref_t::compare(const var_ref_t &r) const
{
  return (int)hexdsp(hx_var_ref_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int ctree_visitor_t::apply_to(citem_t *item, citem_t *parent)
{
  return (int)hexdsp(hx_ctree_visitor_t_apply_to, this, item, parent);
}

//--------------------------------------------------------------------------
inline int ctree_visitor_t::apply_to_exprs(citem_t *item, citem_t *parent)
{
  return (int)hexdsp(hx_ctree_visitor_t_apply_to_exprs, this, item, parent);
}
...
{
  return (int)hexdsp(hx_citem_locator_t_compare, this, &r);
}
...
{
  return (int)hexdsp(hx_cexpr_t_compare, this, &r);
}
...
//--------------------------------------------------------------------------
inline bool cexpr_t::contains_operator(ctype_t needed_op) const
{
  return (uchar)(size_t)hexdsp(hx_cexpr_t_contains_operator, this, needed_op) != 0;
}

//--------------------------------------------------------------------------
inline int cexpr_t::get_high_nbit_bound(int pbits, type_sign_t psign, bool *p_maybe_negative) const
{
  return (int)hexdsp(hx_cexpr_t_get_high_nbit_bound, this, pbits, psign, p_maybe_negative);
}

//--------------------------------------------------------------------------
inline int cexpr_t::get_low_nbit_bound(type_sign_t psign, bool *p_maybe_negative) const
{
  return (int)hexdsp(hx_cexpr_t_get_low_nbit_bound, this, psign, p_maybe_negative);
}
...
{
  return (int)hexdsp(hx_cif_t_compare, this, &r);
}
...
{
  return (int)hexdsp(hx_cfor_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int cwhile_t::compare(const cwhile_t &r) const
{
  return (int)hexdsp(hx_cwhile_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int cdo_t::compare(const cdo_t &r) const
{
  return (int)hexdsp(hx_cdo_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int creturn_t::compare(const creturn_t &r) const
{
  return (int)hexdsp(hx_creturn_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int cgoto_t::compare(const cgoto_t &r) const
{
  return (int)hexdsp(hx_cgoto_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int casm_t::compare(const casm_t &r) const
{
  return (int)hexdsp(hx_casm_t_compare, this, &r);
}
...
{
  return (int)hexdsp(hx_cinsn_t_compare, this, &r);
}
...
//--------------------------------------------------------------------------
inline bool cinsn_t::contains_insn(ctype_t type) const
{
  return (uchar)(size_t)hexdsp(hx_cinsn_t_contains_insn, this, type) != 0;
}
...
{
  return (int)hexdsp(hx_cblock_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int carglist_t::compare(const carglist_t &r) const
{
  return (int)hexdsp(hx_carglist_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int ccase_t::compare(const ccase_t &r) const
{
  return (int)hexdsp(hx_ccase_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int ccases_t::compare(const ccases_t &r) const
{
  return (int)hexdsp(hx_ccases_t_compare, this, &r);
}

//--------------------------------------------------------------------------
inline int cswitch_t::compare(const cswitch_t &r) const
{
  return (int)hexdsp(hx_cswitch_t_compare, this, &r);
}
...
{
  return (int)hexdsp(hx_ctree_item_t_get_label_num, this, gln_flags);
}
...
//--------------------------------------------------------------------------
inline cexpr_t *dereference(cexpr_t *e, int ptrsize, bool is_float)
{
  return (cexpr_t *)hexdsp(hx_dereference, e, ptrsize, is_float);
}
...
{
  return (int32)hexdsp(hx_cfunc_t_get_user_iflags, this, &loc);
}
...
{
  return (int)hexdsp(hx_cfunc_t_del_orphan_cmts, this);
}
...
{
  return (int)hexdsp(hx_remove_hexrays_callback, callback, ud);
}
...
//--------------------------------------------------------------------------
inline cnumber_t *vdui_t::get_number(void) const
{
...
{
  return (int)hexdsp(hx_vdui_t_get_current_label, this);
}
...
{
  return (cmt_type_t)hexdsp(hx_vdui_t_calc_cmt_type, this, lnnum, cmttype);
}

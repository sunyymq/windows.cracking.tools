...
#define FF_OWRD 0x70000000L             // octaword (16 bytes/128 bits)
...
#define isYwrd(F)     (isData(F) && (F & DT_TYPE) == FF_YWRD)
...
string  GetDisasmEx     (long ea, long flags);  // get disassembly line
#endif
...
success AddSegEx(long startea, long endea, long sel, long use32, long align, long comb, long flags);
...
#define FUNCATTR_FRAME   10     // readonly: function frame id
#define FUNCATTR_FRSIZE  14     // readonly: size of local variables
#define FUNCATTR_FRREGS  18     // readonly: size of saved registers area
#define FUNCATTR_ARGSIZE 20     // readonly: number of bytes purged from the stack
#define FUNCATTR_FPD     24     // frame pointer delta
#define FUNCATTR_COLOR   28     // function color code
#define FUNCATTR_OWNER   10     // readonly: chunk owner (valid only for tail chunks)
#define FUNCATTR_REFQTY  14     // readonly: number of chunk parents (valid only for tail chunks)
#else
...
typeinfo GetTinfo(string type_name);
...
success ApplyType(long ea, typeinfo type, long flags);
...
long SetType(long ea, string type);
...
typeinfo GetLocalTinfo(long ordinal);
...
#endif
#define AddBpt(ea) AddBptEx(ea, 0, BPT_DEFAULT) // Shorthand for code breakpoints (default type)
...
#define AddSeg(a1, a2, sel, use32, align, comb) AddSegEx(a1, a2, sel, use32, align, comb, ADDSEG_NOSREG)
...
#define MakeYword(ea)               MakeData(ea, FF_YWRD, 32, BADADDR)
...
#define GetLocalType(ordinal,flags) GetLocalTinfo(ordinal).print(flags)
#define GetDisasm(ea) GetDisasmEx((ea), 0)
